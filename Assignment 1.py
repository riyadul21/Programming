...
def Main_Menu():
    ...
    if userOptions == 1:
         ...
         last_letter = input ("Enter the first letter of their last name")

         i = 0
         amt_tries = len(userList)
         while last_letter not in userList[i][1][0]:
               i += 1
               if i == amt_tries:
                  print("Last name not found!")
                  Main_Menu
         if last_letter in userList[i][1][0]:
            print("Last name has been found")
            print("\nFirst name", userList[i][0], "\nLast name", userList[i][1], "\nAddress", userList[i][2],
            "\n", userList[i][3], "\n", userList[i][4], "\n", userList[i][5], "\nPost Code", userList[i][6],
